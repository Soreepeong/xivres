...
void xivres::base_unpacker::block_decoder::forward(uint32_t lengthToSkip) {
...
void xivres::base_unpacker::block_decoder::forward(const uint32_t requestOffset, const stream& strm, const uint32_t blockOffset, const size_t knownBlockSize) {
	...
	if (m_relativeOffset < block_header().DecompressedSize) {
		...
		if (block_header().compressed()) {
			if (sizeof block_header() + block_header().CompressedSize > read.size_bytes())
				throw bad_data_error("Failed to read block");

			if (m_relativeOffset) {
				const auto buf = m_inflater(read.subspan(sizeof block_header(), block_header().CompressedSize), block_header().DecompressedSize);
				...
				const auto buf = m_inflater(read.subspan(sizeof block_header(), block_header().CompressedSize), target);
			...
			std::copy_n(&read[static_cast<size_t>(sizeof block_header() + m_relativeOffset)], target.size(), target.begin());
		}
