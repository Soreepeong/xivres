...
void xivres::util::bitmap_copy::to_rgba8888::draw_line_to_rgb_opaque(RGBA8888* pTarget, const uint8_t* pSource, size_t nPixelCount) {
	while (nPixelCount--) {
...
void xivres::util::bitmap_copy::to_rgba8888::draw_line_to_rgb(RGBA8888* pTarget, const uint8_t* pSource, size_t nPixelCount) {
	while (nPixelCount--) {
		const auto opacityScaled = m_gammaTable[*pSource];
