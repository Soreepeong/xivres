...
namespace xivres::util {
	namespace implementation_listener_manager {
		...
		public:
			...
			[[nodiscard]] virtual on_dtor operator() (std::function<R(T ...)> fn, std::function<void()> onUnbind = {}) {
				std::lock_guard lock(*m_lock);
				...
				return on_dtor([destructed = m_destructed, onUnbind = std::move(onUnbind), mutex = m_lock, callbackId, this]() {
		...
		public:
			...
			/// \brief Adds a callback function to call when an event has been fired.
			/// \returns An object that will remove the callback when destructed.
			[[nodiscard]] on_dtor operator() (std::function<R(T ...)> fn, std::function<void()> onUnbind = {}) override {
	...
	template <typename F, typename R, typename...T>
	class listener_manager : public implementation_listener_manager::ListenerManagerImpl_<F, R, void, T...> {
	public:
		using implementation_listener_manager::ListenerManagerImpl_<F, R, void, T...>::ListenerManagerImpl_;
	};
