...
namespace xivres {
	...
	public:
		TexturePackedFileStreamDecoder(const PackedFileHeader& header, std::shared_ptr<const packed_stream> strm)
			: BasePackedFileStreamDecoder(std::move(strm)) {
			uint64_t readOffset = sizeof PackedFileHeader;
			const auto locators = m_stream->read_vector<PackedLodBlockLocator>(readOffset, header.BlockCountOrVersion);
