...
namespace xivres {
	...
	public:
		...
		equipment_deformer_parameter_header& header() {
			return *reinterpret_cast<equipment_deformer_parameter_header*>(m_data.data());
		}

		const equipment_deformer_parameter_header& header() const {
		...
		uint16_t setinfo(size_t setId) const {
			return get_setinfo(setId);
		}

		uint16_t get_setinfo(size_t setId) const {
			const auto b = block(setId / header().BlockMemberCount);
			if (b.empty())
				return 0;
			return b[setId % header().BlockMemberCount];
