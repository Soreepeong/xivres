...
namespace xivres {
	...
	public:
		ModelPackedFileStreamDecoder(const PackedFileHeader& header, std::shared_ptr<const packed_stream> strm)
			: BasePackedFileStreamDecoder(std::move(strm))
		{
			...
			const auto locator = m_stream->read_fully<SqpackModelPackedFileBlockLocator>(readOffset);
