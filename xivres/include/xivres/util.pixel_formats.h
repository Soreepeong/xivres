...
namespace xivres::util {
	...
	struct RGBACommon {
		...
		void SetFromF(TR r, TG g, TB b, TA a = 1.) {
			SetFrom(
				static_cast<T>(std::round(static_cast<TR>(MaxR) * util::clamp<TR>(r, 0, 1))),
				static_cast<T>(std::round(static_cast<TG>(MaxG) * util::clamp<TG>(g, 0, 1))),
				static_cast<T>(std::round(static_cast<TB>(MaxB) * util::clamp<TB>(b, 0, 1))),
				static_cast<T>(std::round(static_cast<TA>(MaxA) * util::clamp<TA>(a, 0, 1))));
		}
