	...
	public:
		...
		public:
			...
			std::span<const HashLocatorT> HashLocators() const {
				return util::span_cast<HashLocatorT>(Data, IndexHeader().HashLocatorSegment.Offset, IndexHeader().HashLocatorSegment.Size, 1);
			}

			std::span<const TextLocatorT> TextLocators() const {
				return util::span_cast<TextLocatorT>(Data, IndexHeader().TextLocatorSegment.Offset, IndexHeader().TextLocatorSegment.Size, 1);
			}

			std::span<const SqpackSegment3Entry> Segment3() const {
				return util::span_cast<SqpackSegment3Entry>(Data, IndexHeader().UnknownSegment3.Offset, IndexHeader().UnknownSegment3.Size, 1);
			}

			const SqpackDataLocator& GetLocatorFromTextLocators(const char* fullPath) const {
				const auto it = std::lower_bound(TextLocators().begin(), TextLocators().end(), fullPath, xiv_path_spec::LocatorComparator());
		...
		protected:
			...
			SqIndexType(const stream& strm, bool strictVerify)
		...
		public:
			const std::span<const SqpackPathHashLocator> PathHashLocators() const {
				return util::span_cast<SqpackPathHashLocator>(Data, IndexHeader().PathHashLocatorSegment.Offset, IndexHeader().PathHashLocatorSegment.Size, 1);
			}

			std::span<const SqpackPairHashLocator> GetPairHashLocators(uint32_t pathHash) const {
				const auto it = std::lower_bound(PathHashLocators().begin(), PathHashLocators().end(), pathHash, xiv_path_spec::LocatorComparator());
				if (it == PathHashLocators().end() || it->PathHash != pathHash)
					throw std::out_of_range(std::format("PathHash {:08x} not found", pathHash));

				return util::span_cast<SqpackPairHashLocator>(Data, it->PairHashLocatorOffset, it->PairHashLocatorSize, 1);
			}
			...
			const SqpackDataLocator& GetLocator(uint32_t pathHash, uint32_t nameHash) const {
				...
				const auto it = std::lower_bound(locators.begin(), locators.end(), nameHash, xiv_path_spec::LocatorComparator());
		...
		protected:
			...
			SqIndex1Type(const stream& strm, bool strictVerify)
				: SqIndexType<SqpackPairHashLocator, SqpackPairHashWithTextLocator>(strm, strictVerify)  {
				if (strictVerify) {
		...
		public:
			const SqpackDataLocator& GetLocator(uint32_t fullPathHash) const {
				const auto it = std::lower_bound(HashLocators().begin(), HashLocators().end(), fullPathHash, xiv_path_spec::LocatorComparator());
		...
		protected:
			...
			SqIndex2Type(const stream& strm, bool strictVerify)
				: SqIndexType<SqpackFullHashLocator, SqpackFullHashWithTextLocator>(strm, strictVerify) {
			}
		};
		...
		private:
			...
			SqDataType(std::shared_ptr<stream> strm, const uint32_t datIndex, bool strictVerify)
		...
		SqpackReader(const std::string& fileName, std::shared_ptr<stream> indexStream1, std::shared_ptr<stream> indexStream2, std::vector<std::shared_ptr<stream>> dataStreams, bool strictVerify = false)
			...
			for (uint32_t i = 0; i < Index1.IndexHeader().TextLocatorSegment.Count; ++i) {
				...
				offsets1.emplace_back(SqpackDataLocator(i, Data[i].Stream->size()), std::make_tuple(UINT32_MAX, UINT32_MAX, static_cast<const char*>(nullptr)));
				offsets2.emplace_back(SqpackDataLocator(i, Data[i].Stream->size()), std::make_tuple(UINT32_MAX, static_cast<const char*>(nullptr)));
			}
			...
			for (size_t curr = 1, prev = 0; curr < offsets1.size(); ++curr, ++prev) {
				...
				if (std::get<2>(offsets1[prev].second))
					EntryInfo.back().path_spec = xiv_path_spec(std::get<2>(offsets1[prev].second));
				else if (std::get<1>(offsets2[prev].second))
					EntryInfo.back().path_spec = xiv_path_spec(std::get<1>(offsets2[prev].second));
				else
					EntryInfo.back().path_spec = xiv_path_spec(
		...
		[[nodiscard]] const SqpackDataLocator& GetLocator1(const xiv_path_spec& pathSpec) const {
			try {
		...
		[[nodiscard]] const SqpackDataLocator& GetLocator2(const xiv_path_spec& pathSpec) const {
			try {
		...
		[[nodiscard]] std::shared_ptr<packed_stream> GetPackedFileStream(const EntryInfoType& info) const {
			return std::make_unique<StreamAsPackedFileViewStream>(info.path_spec, std::make_shared<partial_view_stream>(Data.at(info.Locator.DatFileIndex).Stream, info.Locator.DatFileOffset(), info.Allocation));
		}

		[[nodiscard]] std::shared_ptr<packed_stream> GetPackedFileStream(const xiv_path_spec& pathSpec) const {
			struct Comparator {
		...
		[[nodiscard]] std::shared_ptr<PackedFileUnpackingStream> GetFileStream(const xiv_path_spec& pathSpec, std::span<uint8_t> obfuscatedHeaderRewrite = {}) const {
