...
namespace xivres {
	class ScdReader {
		...
		[[nodiscard]] std::vector<uint8_t> ReadEntry(const std::span<const uint32_t>& offsets, uint32_t endOffset, size_t index) const {
			...
			return m_stream->read_vector<uint8_t>(offsets[index], next - offsets[index]);
		}
		...
		[[nodiscard]] static std::vector<uint8_t> GetHeaderBytes(const stream& strm) {
	...
	public:
		ScdReader(std::shared_ptr<stream> strm)
