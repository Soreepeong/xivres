...
namespace xivres {
	class TextureStream : public default_base_stream {
	...
	public:
		TextureStream(const std::shared_ptr<stream>& strm)
			: m_header(strm->read_fully<TextureHeader>(0))
			, m_repeats(0)
		...
		void SetMipmap(size_t mipmapIndex, size_t repeatIndex, std::shared_ptr<MipmapStream> mipmap) {
			...
			if (mipmap->size() != TextureRawDataLength(mipmap->Type, w, h, l))
