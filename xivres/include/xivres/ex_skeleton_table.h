...
namespace xivres {
	...
	public:
		struct descriptor_type {
			...
			bool operator<(const descriptor_type& r) const {
	...
	public:
		...
		ex_skeleton_table_file(const std::map<descriptor_type, uint16_t>& pairs)
			: m_data(4) {
			from_pairs(pairs);
		}
		...
		descriptor_type& descriptor(size_t index) {
			return reinterpret_cast<descriptor_type*>(&m_data[4])[index];
		}

		const descriptor_type& descriptor(size_t index) const {
			return reinterpret_cast<const descriptor_type*>(&m_data[4])[index];
		}

		std::span<descriptor_type> descriptors() {
			return { reinterpret_cast<descriptor_type*>(&m_data[4]), count() };
		}

		std::span<const descriptor_type> descriptors() const {
			return { reinterpret_cast<const descriptor_type*>(&m_data[4]), count() };
		}

		uint16_t& skel_id(size_t index) {
			return reinterpret_cast<uint16_t*>(&m_data[4 + descriptors().size_bytes()])[index];
		}

		const uint16_t& skel_id(size_t index) const {
			return reinterpret_cast<const uint16_t*>(&m_data[4 + descriptors().size_bytes()])[index];
		}

		std::span<uint16_t> skel_ids() {
			return { reinterpret_cast<uint16_t*>(&m_data[4 + descriptors().size_bytes()]), count() };
		}

		std::span<const uint16_t> skel_ids() const {
			return { reinterpret_cast<const uint16_t*>(&m_data[4 + descriptors().size_bytes()]), count() };
		}

		std::map<descriptor_type, uint16_t> to_pairs() const {
			std::map<descriptor_type, uint16_t> res;
			for (size_t i = 0, i_ = count(); i < i_; ++i)
				res.emplace(descriptor(i), skel_id(i));
			return res;
		}

		void from_pairs(const std::map<descriptor_type, uint16_t>& pairs) {
