...
namespace xivres {
	...
	public:
		...
		struct AddEntryResult {
			...
			std::vector<std::pair<xiv_path_spec, std::string>> Error;
			...
			[[nodiscard]] packed_stream* AnyItem() const {
				if (!Added.empty())
					return Added[0];
				if (!Replaced.empty())
					return Replaced[0];
			...
			[[nodiscard]] std::vector<packed_stream*> AllSuccessfulEntries() const {
		...
		struct SqpackViews {
			...
			std::map<xiv_path_spec, std::unique_ptr<Entry>, xiv_path_spec::AllHashComparator> HashOnlyEntries;
			std::map<xiv_path_spec, std::unique_ptr<Entry>, xiv_path_spec::FullPathComparator> FullPathEntries;
		};
	...
	private:
		std::map<xiv_path_spec, std::unique_ptr<Entry>, xiv_path_spec::AllHashComparator> m_hashOnlyEntries;
		std::map<xiv_path_spec, std::unique_ptr<Entry>, xiv_path_spec::FullPathComparator> m_fullEntries;

		std::vector<SqpackSegment3Entry> m_sqpackIndexSegment3;
		std::vector<SqpackSegment3Entry> m_sqpackIndex2Segment3;

	public:
		util::listener_manager<SqpackGenerator, void, const std::string&> Log;
		...
		void AddEntry(AddEntryResult& result, std::shared_ptr<packed_stream> provider, bool overwriteExisting) {
		...
		AddEntryResult AddEntry(std::shared_ptr<packed_stream> provider, bool overwriteExisting = true) {
		...
		AddEntryResult AddEntryFromFile(xiv_path_spec pathSpec, const std::filesystem::path& path, bool overwriteExisting = true) {
			std::shared_ptr<packed_stream> provider;
			...
			if (file_size(path) == 0) {
				...
				provider = std::make_shared<passthrough_packed_stream<texture_passthrough_packer>>(std::move(pathSpec), path);
			} else if (extensionLower == u8".mdl") {
				provider = std::make_shared<passthrough_packed_stream<model_passthrough_packer>>(std::move(pathSpec), path);
			} else {
				provider = std::make_shared<passthrough_packed_stream<standard_passthrough_packer>>(std::move(pathSpec), path);
			}

			return AddEntry(provider, overwriteExisting);
		}

		void ReserveSwappableSpace(xiv_path_spec pathSpec, uint32_t size) {
		...
		SqpackViews FinishToStreams(bool strict, const std::shared_ptr<SqpackViewEntryCache>& dataBuffer = nullptr) {
			...
			for (size_t i = 0; i < res.Entries.size(); ++i) {
				...
				if (dataSubheaders.empty() ||
					...
					if (strict && !dataSubheaders.empty()) {
						...
						dataSubheaders.back().Sha1.set_from_span(reinterpret_cast<char*>(&dataSubheaders.back()), offsetof(SqpackDataHeader, Sha1));
					}
			...
			if (strict)
				dataHeader.Sha1.set_from_span(reinterpret_cast<char*>(&dataHeader), offsetof(SqpackHeader, Sha1));

			res.Index1 = std::make_shared<xivres::memory_stream>(ExportIndexFileData<xivres::SqpackIndexHeader::IndexType::Index, SqpackPairHashLocator, SqpackPairHashWithTextLocator, true>(
			...
			res.Index2 = std::make_shared<xivres::memory_stream>(ExportIndexFileData<xivres::SqpackIndexHeader::IndexType::Index, SqpackFullHashLocator, SqpackFullHashWithTextLocator, false>(
		...
		void FinishToFiles(const std::filesystem::path& dir, bool strict = false) {
			...
			std::map<std::pair<uint32_t, uint32_t>, std::vector<Entry*>> pairHashes;
			std::map<uint32_t, std::vector<Entry*>> fullHashes;
			std::map<Entry*, xiv_path_spec> entrypath_specs;
		...
		[[nodiscard]] std::unique_ptr<default_base_stream> GetFile(const xiv_path_spec& pathSpec) const {
			if (const auto it = m_hashOnlyEntries.find(pathSpec); it != m_hashOnlyEntries.end())
		...
		std::vector<xiv_path_spec> Allpath_spec() const {
			std::vector<xiv_path_spec> res;
	...
	private:
		template<xivres::SqpackIndexHeader::IndexType IndexType, typename packed_type, typename ConflictEntryType, bool UseFolders>
		static std::vector<uint8_t> ExportIndexFileData(
			size_t dataFilesCount,
			std::vector<packed_type> fileSegment,
			...
			if (strict)
				header1.Sha1.set_from_span(reinterpret_cast<char*>(&header1), offsetof(SqpackHeader, Sha1));
			...
			if (strict) {
				header2.Sha1.set_from_span(reinterpret_cast<char*>(&header2), offsetof(SqpackIndexHeader, Sha1));
	...
	class SqpackGenerator::DataViewStream : public default_base_stream {
