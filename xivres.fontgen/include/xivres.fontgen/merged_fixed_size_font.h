...
namespace xivres::fontgen {
	...
	class merged_fixed_size_font : public default_abstract_fixed_size_font {
		...
		std::vector<std::shared_ptr<xivres::fontgen::fixed_size_font>> m_fonts;
	...
	private:
		static int get_vertical_adjustment(const info_t& info, const xivres::fontgen::fixed_size_font& font);
	};
}
