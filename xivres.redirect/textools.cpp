...
void textools::to_json(nlohmann::json& j, const mod_pack_entry_t& p) {
...
void textools::from_json(const nlohmann::json& j, mod_pack_entry_t& p) {
	if (j.is_null())
		return;
	if (!j.is_object())
		throw xivres::bad_data_error("ModPackEntry must be an object");
...
void textools::to_json(nlohmann::json& j, const mod_entry_t& p) {
	j = nlohmann::json::object({
		{"Name", p.Name},
		{"Category", p.Category},
		{"FullPath", p.FullPath},
...
void textools::from_json(const nlohmann::json& j, mod_entry_t& p) {
	if (!j.is_object())
		throw xivres::bad_data_error("ModEntry must be an object");
...
void textools::mod_pack_page::to_json(nlohmann::json& j, const option_t& p) {
	j = nlohmann::json::object({
		{"Name", p.Name},
...
void textools::mod_pack_page::from_json(const nlohmann::json& j, option_t& p) {
	if (!j.is_object())
		throw xivres::bad_data_error("Option must be an object");
...
void textools::mod_pack_page::to_json(nlohmann::json& j, const mod_group_t& p) {
	j = nlohmann::json::object({
		{"GroupName", p.GroupName},
		{"SelectionType", p.SelectionType},
...
void textools::mod_pack_page::from_json(const nlohmann::json& j, mod_group_t& p) {
	if (!j.is_object())
		throw xivres::bad_data_error("Option must be an object");
...
void textools::mod_pack_page::to_json(nlohmann::json& j, const page_t& p) {
	j = nlohmann::json::object({
...
void textools::mod_pack_page::from_json(const nlohmann::json& j, page_t& p) {
	if (!j.is_object())
		throw xivres::bad_data_error("Option must be an object");
...
void textools::to_json(nlohmann::json& j, const ttmpl_t& p) {
	j = nlohmann::json::object({
...
void textools::from_json(const nlohmann::json& j, ttmpl_t& p) {
	if (!j.is_object())
		throw xivres::bad_data_error("TTMPL must be an object");
...
void textools::ttmpl_t::for_each(std::function<void(textools::mod_entry_t&)> cb, const nlohmann::json& choices) {
...
void textools::ttmpl_t::for_each(std::function<void(const textools::mod_entry_t&)> cb, const nlohmann::json& choices) const {
	static const nlohmann::json emptyChoices;
...
bool textools::mod_entry_t::is_textools_metadata() const {
...
const srell::u8cregex textools::metafile::CharacterMetaPathTest(
...
const srell::u8cregex textools::metafile::HousingMetaPathTest(
...
textools::metafile::metafile(std::string gamePath, const xivres::stream& stream)
	...
	if (srell::u8csmatch matches;
		...
		throw xivres::bad_data_error("Unsupported meta file");
	}
...
void textools::metafile::apply_image_change_data_edits(std::function<xivres::image_change_data::file& ()> reader) const {
	if (const auto imcedit = get_span<xivres::image_change_data::entry>(meta_data_type_t::Imc); !imcedit.empty()) {
		auto& imc = reader();
		using imc_t = xivres::image_change_data::image_change_data_type;
...
void textools::metafile::apply_equipment_deformer_parameter_edits(std::function<xivres::equipment_deformer_parameter_file& (item_type_t, uint32_t)> reader) const {
...
bool textools::metafile::has_equipment_parameter_edits() const {
	return !get_span<uint8_t>(textools::metafile::meta_data_type_t::Eqp).empty();
}

void textools::metafile::apply_equipment_parameter_edits(xivres::equipment_parameter_file& eqp) const {
	if (const auto eqpedit = get_span<uint8_t>(textools::metafile::meta_data_type_t::Eqp); !eqpedit.empty()) {
		if (eqpedit.size() != EqpEntrySize)
			throw xivres::bad_data_error(std::format("expected {}b for eqp; got {}b", EqpEntrySize, eqpedit.size()));
		std::copy_n(&eqpedit[0], EqpEntrySize, &eqp.paramter_bytes(PrimaryId)[EqpEntryOffset]);
	}
}

bool textools::metafile::has_gimmick_parameter_edits() const {
	return !get_span<uint8_t>(textools::metafile::meta_data_type_t::Gmp).empty();
}

void textools::metafile::apply_gimmick_parameter_edits(xivres::gimmmick_parameter_file& gmp) const {
	if (const auto gmpedit = get_span<uint8_t>(textools::metafile::meta_data_type_t::Gmp); !gmpedit.empty()) {
		if (gmpedit.size() != sizeof uint64_t)
			throw xivres::bad_data_error(std::format("gmp data must be 8 bytes; {} byte(s) given", gmpedit.size()));
		std::copy_n(&gmpedit[0], gmpedit.size(), &gmp.paramter_bytes(PrimaryId)[0]);
	}
}

void textools::metafile::apply_ex_skeleton_table_edits(xivres::ex_skeleton_table_file& est) const {
	if (const auto estedit = get_span<textools::metafile::ex_skeleton_table_entry_t>(textools::metafile::meta_data_type_t::Est); !estedit.empty()) {
		auto estpairs = est.to_pairs();
		for (const auto& v : estedit) {
			const auto key = xivres::ex_skeleton_table_file::descriptor_t{ .SetId = v.SetId, .RaceCode = v.RaceCode };
			if (v.SkelId == 0)
				estpairs.erase(key);
			else
